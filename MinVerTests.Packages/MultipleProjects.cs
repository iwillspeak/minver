...
""";
        ...
        var versionCalculations = standardOutput
            ...
            .Where(line => line.StartsWith("MinVer: Calculated version ", StringComparison.OrdinalIgnoreCase) || line.StartsWith("MinVer: Skipping task execution and using cached version ", StringComparison.OrdinalIgnoreCase));

        Assert.Collection(
            versionCalculations,
            message => Assert.Equal("MinVer: Calculated version 2.3.4.", message),
            message => Assert.Equal("MinVer: Calculated version 5.6.7.", message),
            message => Assert.Equal("MinVer: Skipping task execution and using cached version 2.3.4", message),
            message => Assert.Equal("MinVer: Skipping task execution and using cached version 5.6.7", message));
